...
The Rust CLI this plugin shells out to lives at [glennib/envoke](https://github.com/glennib/envoke). Consult its README when the plugin's behaviour depends on envoke CLI semantics (tags, overrides, `--format`, exit codes, JSON output shape). If a local checkout exists alongside this repo, prefer reading it over fetching — ask the user for the path.

## Architecture

Mise loads this plugin via two hook files that each define a method on the global `PLUGIN` table:

- `hooks/mise_env.lua` — `PLUGIN:MiseEnv(ctx)`. The workhorse. Reads plugin options from `ctx.options` (set by the consumer in `mise.toml`), parses `.envoke-env` (see format below), builds an `envoke render <env> --config <cfg> --format json --quiet [--tag …] [--override …]` command, executes it via the injected `cmd` module, parses the resulting JSON, and returns `{ env = [...], cacheable = true, watch_files = [...] }`. Warnings and logs use the `[envoke] …` prefix so mise output is greppable. **The hook never calls `error(...)` on the `MiseEnv` path** — a fatal hook error aborts the entire shell activation (breaking downstream init like the prompt), not just this plugin. Every failure mode degrades to `log.warn` + an empty env return so shell init continues. The failure modes split on whether a watched file reflects the fix: missing `.envoke-env`, missing `envoke.yaml`, and malformed `.envoke-env` all return `cacheable = true` because the env-file and config are in `watch_files` (recovery is automatic once the user edits). Envoke-CLI failures (binary missing, non-zero exit, non-JSON output) return `cacheable = false` because no watched file tracks the envoke binary — otherwise a cached empty env would persist even after the user upgrades envoke. Requires envoke >= 2.0.0 (the `render` subcommand and `--format json` preset landed in v2).
